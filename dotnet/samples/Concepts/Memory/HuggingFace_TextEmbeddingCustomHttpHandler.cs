...
public class HuggingFace_TextEmbeddingCustomHttpHandler(ITestOutputHelper output) : BaseTest(output)
{
    ...
    public async Task RunInferenceApiEmbeddingCustomHttpHandlerAsync()
    {
        ...
        var skMemory = new MemoryBuilder().WithTextEmbeddingGeneration(hf).
    ...
    private sealed class CustomHttpClientHandler : HttpClientHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            Stream originalContent = await response.Content.ReadAsStreamAsync(cancellationToken).
                ConfigureAwait(false);

            List<List<List<ReadOnlyMemory<float>>>> modifiedContent = (await JsonSerializer.DeserializeAsync<List<List<List<ReadOnlyMemory<float>>>>>(originalContent, _jsonOptions, cancellationToken).
                ConfigureAwait(false))!;

            Stream modifiedStream = new MemoryStream();

            await JsonSerializer.SerializeAsync(modifiedStream, modifiedContent[0][0].
                    ToList(), _jsonOptions, cancellationToken).
                ConfigureAwait(false);
