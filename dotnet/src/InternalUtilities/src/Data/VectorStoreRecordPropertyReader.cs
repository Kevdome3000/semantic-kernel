...
internal static class VectorStoreRecordPropertyReader
{
    ...
    {
        var keyProperties = definition.Properties.OfType<VectorStoreRecordKeyProperty>().
            ToList();
        ...
        var dataProperties = definition.Properties.OfType<VectorStoreRecordDataProperty>().
            ToList();

        var vectorProperties = definition.Properties.OfType<VectorStoreRecordVectorProperty>().
            ToList();
    ...
    {
        var cache = supportsMultipleVectors
            ? s_multipleVectorsPropertiesCache
            : s_singleVectorPropertiesCache;
    ...
    public static void VerifyPropertyTypes(
        List<PropertyInfo> properties,
        HashSet<Type> supportedTypes,
        string propertyCategoryDescription,
        bool? supportEnumerable = false)
    {
    ...
    public static void VerifyPropertyTypes(
        List<PropertyInfo> properties,
        HashSet<Type> supportedTypes,
        HashSet<Type> supportedEnumerableTypes,
        string propertyCategoryDescription)
    {
        foreach (var property in properties)
        {
            VerifyPropertyType(property.Name, property.PropertyType, supportedTypes, supportedEnumerableTypes,
        ...
        string propertyCategoryDescription)
    {
        foreach (var property in properties)
        {
            VerifyPropertyType(property.DataModelPropertyName, property.PropertyType, supportedTypes, supportedEnumerableTypes,
                propertyCategoryDescription);
        }
    }
        ...
        string propertyCategoryDescription)
    {
        ...
        if (supportedEnumerableTypes.Count > 0 && typeof(IEnumerable).IsAssignableFrom(propertyType))
        {
            var typeToCheck = propertyType switch
            {
                ...
                var interfaceType when interfaceType.GetInterfaces().
                        FirstOrDefault(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IEnumerable<>)) is Type enumerableInterface =>
