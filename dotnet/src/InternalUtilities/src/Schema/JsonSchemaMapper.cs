    ...
    static partial class JsonSchemaMapper
{
    ...
    public static JsonObject GetJsonSchema(this JsonSerializerOptions options, MethodBase method, JsonSchemaMapperConfiguration? configuration = null)
    {
        ...
        foreach (ParameterInfo parameter in method.GetParameters())
        {
            ...
            ResolveParameterInfo(parameter, parameterInfo, ref state, ref parameterDescription,
                ref hasDefaultValue, ref defaultValue, ref isNullableReferenceType, ref isRequired);
        ...
        return CreateSchemaDocument(ref state, title: title, description: description, schemaType: type,
            properties: paramSchemas, requiredProperties: requiredParams);
    }
    ...
    public static string ToJsonString(this JsonNode? node, bool writeIndented = false)
    {
        return node is null
            ? "null"
            : node.ToJsonString(writeIndented
        ...
        Type? parentNullableOfT = null)
    {
        ...
        if (isCacheable && state.TryGetGeneratedSchemaPath(type, parentNullableOfT, customConverter, isNullableReferenceType,
                customNumberHandling, out string? typePath))
        {
        ...
        {
            description ??= type.GetCustomAttribute<DescriptionAttribute>()?.
                Description;
        }
        ...
        if (isCacheable && typeInfo.Kind != JsonTypeInfoKind.None)
        {
            ...
            state.RegisterTypePath(type, parentNullableOfT, customConverter, isNullableReferenceType,
        ...
        switch (typeInfo.Kind)
        {
            ...
            case JsonTypeInfoKind.Object:
                ...
                foreach (JsonPropertyInfo property in typeInfo.Properties)
                {
                    ...
                    NullabilityInfoContext? nullabilityCtx = !property.PropertyType.IsValueType
                    ...
                    string? propertyDescription = state.Configuration.ResolveDescriptionAttributes
                        ? attributeProvider?.GetCustomAttributes(inherit: true).
        ...
        ref bool isRequired)
    {
        ...
        if (state.Configuration.ResolveDescriptionAttributes)
        {
            ...
            description ??= parameter.GetCustomAttribute<DescriptionAttribute>()?.
                Description;
        }
    ...
    private ref struct GenerationState
    {
        ...
        public GenerationState(JsonSchemaMapperConfiguration configuration)
        {
            ...
            _nullabilityInfoContext = configuration.ReferenceTypeNullability is ReferenceTypeNullability.Annotated
                ? new()
                : null;

            _generatedTypePaths = configuration.AllowSchemaReferences
                ? new()
                : null;

            _currentPath = configuration.AllowSchemaReferences
                ? new()
                : null;
        ...
        /// </summary>
        public readonly void RegisterTypePath(
            Type type,
            Type? parentNullableOfT,
            JsonConverter? customConverter,
            bool isNullableReferenceType,
            JsonNumberHandling? customNumberHandling)
        {
            if (Configuration.AllowSchemaReferences)
            {
                Debug.Assert(_currentPath is not null);
        ...
        /// </summary>
        public readonly bool TryGetGeneratedSchemaPath(
            Type type,
            Type? parentNullableOfT,
            JsonConverter? customConverter,
            bool isNullableReferenceType,
