...
internal readonly struct SseLine : IEquatable<SseLine>
{
    ...
    internal SseLine(
        string original,
        int colonIndex,
        bool hasSpaceAfterColon,
        string? lastEventName)
    {
        this._original = original;
        this._colonIndex = colonIndex;

        this._valueIndex = colonIndex >= 0
    ...
    /// </summary>
    public ReadOnlyMemory<char> FieldName => this._colonIndex >= 0
        ? this._original.AsMemory(0, this._colonIndex)
        : this._original.AsMemory();

    /// <summary>
    /// Represents a field value in Server-Sent Events (SSE) format.
    /// </summary>
    public ReadOnlyMemory<char> FieldValue => this._valueIndex >= 0
        ? this._original.AsMemory(this._valueIndex)
