...
#pragma warning restore SKEXP0005
        ...
        Assert.True(binaryContent.Content?.ToArray().
            SequenceEqual(new BinaryData(new[] { 1, 2, 3 }).ToArray()));
...
#pragma warning restore SKEXP0005 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
        ...
        Assert.True(audioContent.Data?.ToArray().
            SequenceEqual(new BinaryData(new[] { 3, 2, 1 }).ToArray()));
        ...
        Assert.True(imageContent.Data?.ToArray().
            SequenceEqual(new BinaryData(new[] { 2, 1, 3 }).ToArray()));
