...
public class DataUriParserTests(ITestOutputHelper output)
{
    ...
    [Theory]
    [InlineData("data:,", "text/plain", "", null,
        "{}")] // Minimum valid data URI
    [InlineData("data:,A%20brief%20note", "text/plain", "A%20brief%20note", null,
        "{}")] // Example in RFC 2397 Doc
    [InlineData("data:text/plain;charset=iso-8859-7,%be%fg%be", "text/plain", "%be%fg%be", null,
        """{"charset":"iso-8859-7"}""")] // Example in RFC 2397 Doc
    [InlineData("""data:image/gif;base64,R0lGODdhMAAwAPAAAAAAAP///ywAAAAAMAAwAAAC8IyPqcvt""", "image/gif", "R0lGODdhMAAwAPAAAAAAAP///ywAAAAAMAAwAAAC8IyPqcvt", "base64",
        "{}")] // Example in RFC 2397 Doc
    [InlineData("data:text/vnd-example+xyz;foo=bar;base64,R0lGODdh", "text/vnd-example+xyz", "R0lGODdh", "base64",
        """{"foo":"bar"}""")]
    [InlineData("data:application/octet-stream;base64,AQIDBA==", "application/octet-stream", "AQIDBA==", "base64",
        "{}")]
    [InlineData("data:text/plain;charset=UTF-8;page=21,the%20data:1234,5678", "text/plain", "the%20data:1234,5678", null,
        """{"charset":"UTF-8","page":"21"}""")]
    [InlineData("data:image/svg+xml;utf8,<svg width='10'... </svg>", "image/svg+xml", "<svg width='10'... </svg>", "utf8",
        "{}")]
    [InlineData("data:;charset=UTF-8,the%20data", "text/plain", "the%20data", null,
        """{"charset":"UTF-8"}""")]
    [InlineData("data:text/vnd-example+xyz;foo=;base64,R0lGODdh", "text/vnd-example+xyz", "R0lGODdh", "base64",
        """{"foo":""}""")]
    public void ItCanParseDataUri(
        string dataUri,
        string? expectedMimeType,
        string expectedData,
        string? expectedDataFormat,
        string serializedExpectedParameters)
    {
