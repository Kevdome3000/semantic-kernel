...
public class OpenApiTest
{
    ...
    public async Task PutLightByIdAsync()
    {
        ...
        string? contentType = this._expectedJson?[Operation]?["ContentType"]?.
            ToString();

        Assert.NotNull(contentType);

        Assert.True(httpMessageHandlerStub?.ContentHeaders?.ContentType?.ToString().
            StartsWith(contentType, System.StringComparison.InvariantCulture));
    ...
    {
        Assert.Equal(this._expectedJson?[operation]?["Method"]?.
            ToString(), httpMessageHandlerStub?.Method?.ToString());

        Assert.Equal(this._expectedJson?[operation]?["Uri"]?.
            ToString(), httpMessageHandlerStub?.RequestUri?.ToString());
    }
