...
public sealed class PineconeClient : IPineconeClient
{
        ...
        HttpClient? httpClient = null)
    {
        ...
        _logger = loggerFactory?.CreateLogger(typeof(PineconeClient)) ?? NullLogger.Instance;
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        string basePath = await GetVectorOperationsApiBasePathAsync(indexName).
        ...
        try
        {
            (_, responseContent) = await ExecuteHttpRequestAsync(basePath, request, cancellationToken).
                ConfigureAwait(false);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await foreach (UpsertRequest? batch in PineconeUtils.GetUpsertBatchesAsync(validVectors, MaxBatchSize).
                           WithCancellation(cancellationToken).
                           ConfigureAwait(false))
        {
            totalBatches++;

            using HttpRequestMessage request = batch.ToNamespace(indexNamespace).
                Build();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await ExecuteHttpRequestAsync(basePath, request, cancellationToken).
                ConfigureAwait(false);
        }
        catch (HttpOperationException e)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using HttpRequestMessage request = UpdateVectorRequest.FromPineconeDocument(document).
            InNamespace(indexNamespace).
            Build();
    ...
    {
        using HttpRequestMessage request = ListIndexesRequest.Create().
            Build();

        (HttpResponseMessage _, string responseContent) = await ExecuteHttpRequestAsync(GetIndexOperationsApiBasePath(), request, cancellationToken).
            ConfigureAwait(false);
    ...
    public async Task CreateIndexAsync(IndexDefinition indexDefinition, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await ExecuteHttpRequestAsync(GetIndexOperationsApiBasePath(), request, cancellationToken).
                ConfigureAwait(false);
        }
    ...
    public async Task DeleteIndexAsync(string indexName, CancellationToken cancellationToken = default)
    {
        ...
        using HttpRequestMessage request = DeleteIndexRequest.Create(indexName).
            Build();
    ...
    public async Task<bool> DoesIndexExistAsync(string indexName, CancellationToken cancellationToken = default)
    {
        ...
        List<string?>? indexNames = await ListIndexesAsync(cancellationToken).
        ...
        PineconeIndex? index = await DescribeIndexAsync(indexName, cancellationToken).
            ConfigureAwait(false);
    ...
    public async Task<PineconeIndex?> DescribeIndexAsync(string indexName, CancellationToken cancellationToken = default)
    {
        ...
        using HttpRequestMessage request = DescribeIndexRequest.Create(indexName).
            Build();
        ...
        try
        {
            (_, responseContent) = await ExecuteHttpRequestAsync(GetIndexOperationsApiBasePath(), request, cancellationToken).
                ConfigureAwait(false);
        }
        catch (HttpOperationException e) when (e.StatusCode == HttpStatusCode.BadRequest)
        {
    ...
    public async Task ConfigureIndexAsync(
        string indexName,
        int replicas = 1,
        PodType podType = PodType.P1X1,
        CancellationToken cancellationToken = default)
    {
        _logger.LogDebug("Configuring index {0}", indexName);

        using HttpRequestMessage request = ConfigureIndexRequest.Create(indexName).
            WithPodType(podType).
    ...
    {
        string indexHost = await GetIndexHostAsync(indexName).
            ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using HttpResponseMessage response = await _httpClient.SendWithSuccessCheckAsync(request, cancellationToken).
            ConfigureAwait(false);

        string responseContent = await response.Content.ReadAsStringWithExceptionMappingAsync().
            ConfigureAwait(false);
    ...
    private async Task<string> GetIndexHostAsync(string indexName, CancellationToken cancellationToken = default)
    {
        ...
        PineconeIndex pineconeIndex = await DescribeIndexAsync(indexName, cancellationToken).
                                          ConfigureAwait(false) ??
