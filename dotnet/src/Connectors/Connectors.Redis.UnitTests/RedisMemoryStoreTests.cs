...
public class RedisMemoryStoreTests
{
    ...
    public async Task ItCanInsertIntoNonExistentCollectionAsync()
    {
        ...
        byte[] embedding = MemoryMarshal.Cast<float, byte>(testRecord.Embedding.Span).
            ToArray();

        this._mockDatabase.Setup<Task>(x => x.HashSetAsync(
            ...
            Callback(() =>
            {
                this._mockDatabase.Setup<Task<HashEntry[]>>(x => x.HashGetAllAsync(It.Is<RedisKey>(x => x == redisKey), It.IsAny<CommandFlags>())).
                    ReturnsAsync(new[]
                    {
    ...
    public async Task GetNearestMatchesReturnsAllResultsWithNoMinScoreAsync()
    {
        ...
        // Act
        var topNResults = store.GetNearestMatchesAsync(collection, compareEmbedding, limit: topN, minRelevanceScore: threshold).
            ToEnumerable().
            ToArray();
    ...
    private void MockCreateIndex(string collection, Action? callback = null)
    {
        ...
        this._mockDatabase.Setup(x => x.CreateBatch(It.IsAny<object>())).
            Returns(mockBatch.Object);

        this._mockDatabase.Setup<Task<RedisResult>>(x => x.ExecuteAsync(
            ...
            Callback(() =>
            {
                ...
                this._mockDatabase.Setup<Task<RedisResult>>(x => x.ExecuteAsync(
                    ...
                    ReturnsAsync(RedisResult.Create(this._collections.Select(x => RedisResult.Create(x.Key, ResultType.BulkString)).
                        ToArray()));

                callback?.Invoke();
            });

        this._mockDatabase.Setup<Task<RedisResult>>(x => x.ExecuteAsync(It.Is<string>(x => x == "FT.INFO"), It.IsAny<object[]>())).
    ...
    private void MockHashSet(string collection, MemoryRecord record, Action? callback = null)
    {
        ...
        byte[] embedding = MemoryMarshal.Cast<float, byte>(record.Embedding.Span).
            ToArray();
    ...
    private void MockKeyDelete(string collection, string key, Action? callback = null)
    {
        ...
        this._mockDatabase.Setup<Task<bool>>(x => x.KeyDeleteAsync(
                It.Is<RedisKey>(x => x == redisKey),
                It.IsAny<CommandFlags>())
            ).
            ReturnsAsync(true).
    ...
    {
        RedisKey[] redisKeys = keys.Distinct().
            Select(key => new RedisKey($"{collection}:{key}")).
            ToArray();

        this._mockDatabase.Setup<Task<long>>(x => x.KeyDeleteAsync(
        ...
        bool returnStringVectorScore = false)
    {
        ...
        embeddings = embeddings.OrderByDescending(l => l.Score).
            Take(topN).
            ToList();
        ...
        foreach (var item in embeddings)
        {
            ...
            byte[] embedding = MemoryMarshal.Cast<float, byte>(item.Record.Embedding.Span).
                ToArray();
        ...
        this._mockDatabase.Setup<Task<RedisResult>>(x => x.ExecuteAsync(
                ...
                It.Is<object[]>(x => x[0].
                        ToString() == collection && x[1].
                        ToString() == $"*=>[KNN {topN} @embedding $embedding AS vector_score]"))
            ).
