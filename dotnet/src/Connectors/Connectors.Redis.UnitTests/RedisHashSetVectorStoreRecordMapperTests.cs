...
public sealed class RedisHashSetVectorStoreRecordMapperTests
{
    ...
    public void MapsAllFieldsFromDataToStorageModel()
    {
        ...
        Assert.Equal("storage_string_data", actual.HashEntries[0].
            Name.ToString());

        Assert.Equal("data 1", actual.HashEntries[0].
            Value.ToString());

        Assert.Equal("IntData", actual.HashEntries[1].
            Name.ToString());

        Assert.Equal(1, (int)actual.HashEntries[1].Value);

        Assert.Equal("UIntData", actual.HashEntries[2].
            Name.ToString());

        Assert.Equal(2u, (uint)actual.HashEntries[2].Value);

        Assert.Equal("LongData", actual.HashEntries[3].
            Name.ToString());

        Assert.Equal(3, (long)actual.HashEntries[3].Value);

        Assert.Equal("ULongData", actual.HashEntries[4].
            Name.ToString());

        Assert.Equal(4ul, (ulong)actual.HashEntries[4].Value);

        Assert.Equal("DoubleData", actual.HashEntries[5].
            Name.ToString());

        Assert.Equal(5.5d, (double)actual.HashEntries[5].Value);

        Assert.Equal("FloatData", actual.HashEntries[6].
            Name.ToString());

        Assert.Equal(6.6f, (float)actual.HashEntries[6].Value);

        Assert.Equal("BoolData", actual.HashEntries[7].
            Name.ToString());

        Assert.True((bool)actual.HashEntries[7].Value);

        Assert.Equal("NullableIntData", actual.HashEntries[8].
            Name.ToString());

        Assert.Equal(7, (int)actual.HashEntries[8].Value);

        Assert.Equal("NullableUIntData", actual.HashEntries[9].
            Name.ToString());

        Assert.Equal(8u, (uint)actual.HashEntries[9].Value);

        Assert.Equal("NullableLongData", actual.HashEntries[10].
            Name.ToString());

        Assert.Equal(9, (long)actual.HashEntries[10].Value);

        Assert.Equal("NullableULongData", actual.HashEntries[11].
            Name.ToString());

        Assert.Equal(10ul, (ulong)actual.HashEntries[11].Value);

        Assert.Equal("NullableDoubleData", actual.HashEntries[12].
            Name.ToString());

        Assert.Equal(11.1d, (double)actual.HashEntries[12].Value);

        Assert.Equal("NullableFloatData", actual.HashEntries[13].
            Name.ToString());

        Assert.Equal(12.2f, (float)actual.HashEntries[13].Value);

        Assert.Equal("NullableBoolData", actual.HashEntries[14].
            Name.ToString());

        Assert.False((bool)actual.HashEntries[14].Value);

        Assert.Equal("FloatVector", actual.HashEntries[15].
            Name.ToString());

        Assert.Equal(new float[] { 1, 2, 3, 4 }, MemoryMarshal.Cast<byte, float>((byte[])actual.HashEntries[15].Value!).
            ToArray());

        Assert.Equal("DoubleVector", actual.HashEntries[16].
            Name.ToString());

        Assert.Equal(new double[] { 5, 6, 7, 8 }, MemoryMarshal.Cast<byte, double>((byte[])actual.HashEntries[16].Value!).
            ToArray());
    }
    ...
    private static HashEntry[] CreateHashSet()
    {
        ...
        hashSet[15] = new HashEntry("FloatVector", MemoryMarshal.AsBytes(new ReadOnlySpan<float>(new float[] { 1, 2, 3, 4 })).
            ToArray());

        hashSet[16] = new HashEntry("DoubleVector", MemoryMarshal.AsBytes(new ReadOnlySpan<double>(new double[] { 5, 6, 7, 8 })).
            ToArray());
