...
public sealed class RedisJsonVectorStoreRecordMapperTests
{
    ...
    public void MapsAllFieldsFromDataToStorageModel()
    {
        ...
        Assert.Equal("data 1", jsonObject?["Data1"]?.
            ToString());

        Assert.Equal("data 2", jsonObject?["Data2"]?.
            ToString());

        Assert.Equal(new float[] { 1, 2, 3, 4 }, jsonObject?["Vector1"]?.
            AsArray().
            GetValues<float>().
            ToArray());

        Assert.Equal(new float[] { 5, 6, 7, 8 }, jsonObject?["Vector2"]?.
            AsArray().
            GetValues<float>().
            ToArray());
    }
    ...
    public void MapsAllFieldsFromStorageToDataModel()
    {
        ...
        jsonObject.Add("Vector1", new JsonArray(new[] { 1, 2, 3, 4 }.Select(x => JsonValue.Create(x)).
            ToArray()));

        jsonObject.Add("Vector2", new JsonArray(new[] { 5, 6, 7, 8 }.Select(x => JsonValue.Create(x)).
            ToArray()));
