...
public class QdrantMemoryStoreTests2
{
    ...
    public async Task GetAsyncCallsDoNotRequestVectorsUnlessSpecifiedAsync()
    {
        ...
        _ = await vectorStore.GetBatchAsync("test_collection", [this._id2]).
            ToListAsync();

        _ = await vectorStore.GetBatchAsync("test_collection", [this._id2], true).
            ToListAsync();
    ...
    public async Task GetBatchAsyncSearchesByMetadataIdReturnsAllResultsIfAllFoundAsync()
    {
        ...
        mockQdrantClient.Setup<Task<QdrantVectorRecord?>>(x => x.GetVectorByPayloadIdAsync(It.IsAny<string>(), this._id, It.IsAny<bool>(), It.IsAny<CancellationToken>())).
            ReturnsAsync(qdrantVectorRecord);

        mockQdrantClient.Setup<Task<QdrantVectorRecord?>>(x => x.GetVectorByPayloadIdAsync(It.IsAny<string>(), this._id2, It.IsAny<bool>(), It.IsAny<CancellationToken>())).
            ReturnsAsync(qdrantVectorRecord2);

        mockQdrantClient.Setup<Task<QdrantVectorRecord?>>(x => x.GetVectorByPayloadIdAsync(It.IsAny<string>(), this._id3, It.IsAny<bool>(), It.IsAny<CancellationToken>())).
            ReturnsAsync(qdrantVectorRecord3);

        var vectorStore = new QdrantMemoryStore(mockQdrantClient.Object, this._mockLogger.Object);

        // Act
        var getBatchResult = await vectorStore.GetBatchAsync("test_collection", [this._id, this._id2, this._id3], false).
            ToListAsync();
    ...
    public async Task GetBatchByQdrantPointIdsReturnsAllResultsIfFoundAsync()
    {
        ...
        // Act
        var getBatchResult = await vectorStore.GetWithPointIdBatchAsync("test_collection", [key, key2, key3], false).
            ToListAsync();
