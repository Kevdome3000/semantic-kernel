...
public class QdrantVectorStoreRecordCollectionTests
{
    ...
    public async Task CollectionExistsReturnsCollectionStateAsync(string collectionName, bool expectedExists)
    {
        ...
        this._qdrantClientMock.Setup(x => x.CollectionExistsAsync(
                It.IsAny<string>(),
    ...
    public async Task CanCreateCollectionAsync()
    {
        ...
        this._qdrantClientMock.Setup(x => x.CreateCollectionAsync(
                It.IsAny<string>(),
                It.IsAny<VectorParams>(),
                this._testCancellationToken)).
            Returns(Task.CompletedTask);

        this._qdrantClientMock.Setup(x => x.CreatePayloadIndexAsync(
                It.IsAny<string>(),
                It.IsAny<string>(),
    ...
    public async Task CanDeleteCollectionAsync()
    {
        ...
        this._qdrantClientMock.Setup(x => x.DeleteCollectionAsync(
                It.IsAny<string>(),
                null,
                this._testCancellationToken)).
            Returns(Task.CompletedTask);
        ...
        where TKey : notnull
    {
        ...
        this.SetupRetrieveMock(testRecordKeys.Select(x => CreateRetrievedPoint(hasNamedVectors, x)).
            ToList());
    ...
    public async Task CanUpsertRecordWithCustomMapperAsync()
    {
        ...
        mapperMock.Setup(x => x.MapFromDataToStorageModel(It.IsAny<SinglePropsModel<ulong>>())).
            Returns(pointStruct);
    ...
    {
        this._qdrantClientMock.Setup(x => x.RetrieveAsync(
                It.IsAny<string>(),
    ...
    {
        this._qdrantClientMock.Setup(x => x.DeleteAsync(
                It.IsAny<string>(),
                It.IsAny<ulong>(),
                It.IsAny<bool>(), // wait
    ...
    {
        this._qdrantClientMock.Setup(x => x.UpsertAsync(
                It.IsAny<string>(),
        ...
        var store = new QdrantVectorStoreRecordCollection<SinglePropsModel<T>>(
            ...
            new()
            {
                VectorStoreRecordDefinition = useDefinition
                    ? CreateSinglePropsDefinition(typeof(T))
                    : null,
    ...
    {
        var counterArray = Enumerable.Range(0, input.Length).
            Select(x => 0).
            ToArray();

        // Add each item from the first option set as a separate row.
        object[][] currentCombinations = input[0].
        ...
        for (int currentOptionSetIndex = 1; currentOptionSetIndex < input.Length; currentOptionSetIndex++)
        {
            ...
            foreach (var currentCombination in currentCombinations)
            {
                ...
                {
                    iterationCombinations.Add(currentCombination.Append(currentOptionSet[currentColumnRow]).
                        ToArray());
                }
            }
