...
#pragma warning restore CA1031
        ...
        string? errorMessage)
    {
        ...
        var message = new GeminiChatMessageContent(AuthorRole.Tool,
            ...
            calledToolResult: functionResponse != null
    ...
    private static void ValidateAndPrepareChatHistory(ChatHistory chatHistory)
    {
        ...
        if (chatHistory.Where(message => message.Role == AuthorRole.System).
                ToList() is { Count: > 0 } systemMessages)
        {
    ...
    private static void ValidateChatHistoryMessagesOrder(ChatHistory chatHistory)
    {
        ...
        ChatHistory chatHistoryCopy = new(chatHistory.Where(message => message.Role != AuthorRole.Tool && (message is not GeminiChatMessageContent { ToolCalls: not null })));

        for (int i = 0; i < chatHistoryCopy.Count; i++)
        {
            if (chatHistoryCopy[i].Role != (i % 2 == 0
                    ? AuthorRole.User
