...
internal sealed class HuggingFaceMessageApiClient
{
      ...
      [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        var request = CreateChatRequest(chatHistory, huggingFaceExecutionSettings, modelId);
        request.Stream = true;

        using var activity = ModelDiagnostics.StartCompletionActivity(endpoint, modelId ?? string.Empty, _clientCore.ModelProvider, chatHistory, huggingFaceExecutionSettings);
        HttpResponseMessage? httpResponseMessage = null;
        Stream? responseStream = null;
        try
        {
            using var httpRequestMessage = _clientCore.CreatePost(request, endpoint, _clientCore.ApiKey);
            httpResponseMessage = await _clientCore.SendRequestAndGetResponseImmediatelyAfterHeadersReadAsync(httpRequestMessage, cancellationToken).ConfigureAwait(false);
        ...
        var responseEnumerator = ProcessChatResponseStreamAsync(responseStream, modelId, cancellationToken)
        ...
        CancellationToken cancellationToken)
    {
        ...
        LogChatCompletionUsage(huggingFaceExecutionSettings, response);
    ...
    private void LogChatCompletionUsage(HuggingFacePromptExecutionSettings executionSettings, ChatCompletionResponse chatCompletionResponse)
    {
        ...
        {
            _clientCore.Logger.LogDebug("Token usage information unavailable.");
            return;
        }

        if (_clientCore.Logger.IsEnabled(LogLevel.Information))
        {
            _clientCore.Logger.LogInformation(
        ...
        string? modelId)
    {
        ...
        if (_clientCore.Logger.IsEnabled(LogLevel.Trace))
        {
            _clientCore.Logger.LogTrace("ChatHistory: {ChatHistory}, Settings: {Settings}",
    ...
    private Uri GetChatGenerationEndpoint()
        => new($"{_clientCore.Endpoint}{_clientCore.Separator}v1/chat/completions");
}
