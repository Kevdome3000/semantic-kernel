    ...
    where TRecord : class
{
    ...
    public async Task CreateCollectionAsync(CancellationToken cancellationToken = default)
    {
        if (!this._options.HasNamedVectors)
        {
            ...
            var singleVectorProperty = this._vectorStoreRecordDefinition.Properties.OfType<VectorStoreRecordVectorProperty>().
                First();
        ...
        var dataProperties = this._vectorStoreRecordDefinition.Properties.OfType<VectorStoreRecordDataProperty>().
        ...
        dataProperties = this._vectorStoreRecordDefinition.Properties.OfType<VectorStoreRecordDataProperty>().
            Where(x => x.IsFullTextSearchable);
    ...
    public async Task<TRecord?> GetAsync(ulong key, GetRecordOptions? options = null, CancellationToken cancellationToken = default)
    {
        ...
        var retrievedPoints = await this.GetBatchAsync([key], options, cancellationToken).
    ...
    public async Task<ulong> UpsertAsync(TRecord record, UpsertRecordOptions? options = default, CancellationToken cancellationToken = default)
    {
        ...
        await this.RunOperationAsync(
                UpsertName,
                () => this._qdrantClient.UpsertAsync(this._collectionName, [pointStruct], true, cancellationToken: cancellationToken)).
            ConfigureAwait(false);
    ...
    public async IAsyncEnumerable<ulong> UpsertBatchAsync(IEnumerable<TRecord> records, UpsertRecordOptions? options = default, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await this.RunOperationAsync(
                UpsertName,
                () => this._qdrantClient.UpsertAsync(this._collectionName, pointStructs, true, cancellationToken: cancellationToken)).
            ConfigureAwait(false);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        // Create options.
        var pointsIds = keys.Select(key => keyConverter(key)).
            ToArray();
        ...
        var retrievedPoints = await this.RunOperationAsync(
                OperationName,
                () => this._qdrantClient.RetrieveAsync(this._collectionName, pointsIds, true, includeVectors,
                    cancellationToken: cancellationToken)).
            ConfigureAwait(false);
