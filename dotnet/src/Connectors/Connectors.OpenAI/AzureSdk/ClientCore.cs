...
internal abstract class ClientCore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using (var activity = ModelDiagnostics.StartCompletionActivity(this.Endpoint, this.DeploymentOrModelName, ModelProvider, prompt,
                   textExecutionSettings))
        {
            ...
            catch (Exception ex) when (activity is not null)
            {
                ...
                if (responseData != null)
                {
                    ...
                    activity.SetResponseId(responseData.Id).
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            response = await RunRequestAsync(() => this.Client.GetCompletionsStreamingAsync(options, cancellationToken)).
        ...
        List<OpenAIStreamingTextContent>? streamedContents = activity is not null
            ? []
            : null;
            ...
            while (true)
            {
                ...
                foreach (Choice choice in completions.Choices)
                {
                    var openAIStreamingTextContent = new OpenAIStreamingTextContent(
                        choice.Text, choice.Index, this.DeploymentOrModelName, choice,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (data.Count > 0)
        {
            ...
            var response = await RunRequestAsync(() => this.Client.GetEmbeddingsAsync(embeddingsOptions, cancellationToken)).
                ConfigureAwait(false);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        for (int requestIndex = 1;; requestIndex++)
        {
                       ...
                       chatExecutionSettings))
            {
                ...
                try
                {
                    response = await RunRequestAsync(() => this.Client.GetChatCompletionsStreamingAsync(chatOptions, cancellationToken)).
                        ConfigureAwait(false);
                }
                catch (Exception ex) when (activity is not null)
                {
