...
public class AzureCosmosDBMongoDBMemoryStore : IMemoryStore, IDisposable
{
    ...
    {
        await this._mongoDatabase.CreateCollectionAsync(
                collectionName,
        ...
        if (!indexes.ToList(cancellationToken: cancellationToken).
                Any(index => index["name"] == this._config.IndexName))
        {
            ...
            await this._mongoDatabase.RunCommandAsync<BsonDocument>(
                    command,
                    cancellationToken: cancellationToken
                ).
                ConfigureAwait(false);
    ...
    )
    {
        ...
        var cosmosRecord = await cursor.SingleOrDefaultAsync(cancellationToken).
            ConfigureAwait(false);
    ...
    )
    {
        ...
        double similarityScore = result.GetValue("similarityScore").
            AsDouble;
    ...
    )
    {
                   ...
                   ConfigureAwait(false))
        {
            ...
            {
                // Access the similarityScore from the BSON document
                var similarityScore = doc.GetValue("similarityScore").
                    AsDouble;
