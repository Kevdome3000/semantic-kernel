...
internal sealed class ChatWithDataResult : IChatResult, ITextResult
{
    ...
    public Task<ChatMessage> GetChatMessageAsync(CancellationToken cancellationToken = default)
    {
        ...
        return message is not null ? Task.FromResult<ChatMessage>(new AzureOpenAIChatMessage(message.Role, message.Content)) : Task.FromException<ChatMessage>(new KernelException("No message found"));
    }
