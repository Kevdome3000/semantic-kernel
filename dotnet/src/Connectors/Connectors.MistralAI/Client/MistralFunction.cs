...
internal sealed partial class MistralFunction
{
    ...
    public MistralFunction(string name, string description, MistralParameters? parameters)
    {
        ...
        this.PluginName = (parts.Length == 1)
            ? null
            : parts[0];

        this.FunctionName = (parts.Length == 1)
            ? parts[0]
            : parts[1];
    ...
    {
        var name = string.IsNullOrEmpty(metadata.PluginName)
            ? metadata.Name
            : $"{metadata.PluginName}{NameSeparator}{metadata.Name}";
    ...
    {
        var name = string.IsNullOrEmpty(pluginName)
            ? functionName
