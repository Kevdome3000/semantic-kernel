    ...
    where TConsumerDataModel : class
{
        ...
        if (options.IncludeVectors)
        {
            foreach (var property in this._vectorPropertiesInfo)
            {
                ...
                if (hashEntry.Name.HasValue)
                {
                    ...
                    {
                        var array = MemoryMarshal.Cast<byte, float>((byte[])hashEntry.Value!).
                            ToArray();
                    ...
                    {
                        var array = MemoryMarshal.Cast<byte, double>((byte[])hashEntry.Value!).
                            ToArray();
    ...
    {
        return MemoryMarshal.AsBytes(vector.Span).
            ToArray();
    }
