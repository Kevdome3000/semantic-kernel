    ...
    where TConsumerDataModel : class
{
    ...
    public (string Key, JsonNode Node) MapFromDataToStorageModel(TConsumerDataModel dataModel)
    {
        ...
        if (jsonNode!.AsObject().
                TryGetPropertyValue(this._keyFieldJsonPropertyName, out var keyField) && keyField is JsonValue jsonValue)
        {
            ...
            jsonNode.AsObject().
                Remove(this._keyFieldJsonPropertyName);
