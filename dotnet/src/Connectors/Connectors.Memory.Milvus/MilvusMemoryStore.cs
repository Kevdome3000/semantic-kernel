...
public class MilvusMemoryStore : IMemoryStore, IDisposable
{
    ...
    {
        await foreach (MemoryRecord record in this.GetBatchAsync(collectionName, [key], withEmbedding, cancellationToken).
                           ConfigureAwait(false))
        {
            return record;
        }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        SearchResults results = await collection.SearchAsync(EmbeddingFieldName, [embedding], SimilarityMetricType.Ip, limit,
