...
public class AzureAISearchVectorStoreRecordCollectionTests
{
    ...
    {
        this._searchIndexClientMock.Setup(x => x.GetSearchClient(collectionName)).
            Returns(this._searchClientMock.Object);

        // Arrange.
        if (expectedExists)
        {
            this._searchIndexClientMock.Setup(x => x.GetIndexAsync(collectionName, this._testCancellationToken)).
    ...
    {
        // Arrange.
        this._searchIndexClientMock.Setup(x => x.CreateIndexAsync(It.IsAny<SearchIndex>(), this._testCancellationToken)).
        ...
        // Assert.
        var expectedFieldNames = useCustomJsonSerializerOptions
            ? new[] { "key", "storage_data1", "data2", "storage_vector1", "vector2" }
            : new[] { "Key", "storage_data1", "Data2", "storage_vector1", "Vector2" };

        this._searchIndexClientMock.Verify(
            x => x.CreateIndexAsync(
                It.Is<SearchIndex>(si => si.Fields.Count == 5 && si.Fields.Select(f => f.Name).
                    SequenceEqual(expectedFieldNames) && si.Name == TestCollectionName && si.VectorSearch.Profiles.Count == 2 && si.VectorSearch.Algorithms.Count == 2),
    ...
    public async Task CreateCollectionIfNotExistsSDKAsync(bool useDefinition, bool expectedExists)
    {
        ...
        if (expectedExists)
        {
            this._searchIndexClientMock.Setup(x => x.GetIndexAsync(TestCollectionName, this._testCancellationToken)).
                Returns(Task.FromResult<Response<SearchIndex>?>(null));
        }
        else
        {
    ...
    {
        // Arrange.
        this._searchIndexClientMock.Setup(x => x.DeleteIndexAsync(TestCollectionName, this._testCancellationToken)).
    ...
    public async Task CanGetRecordWithoutVectorsAsync(bool useDefinition, bool useCustomJsonSerializerOptions)
    {
        ...
        var expectedSelectFields = useCustomJsonSerializerOptions
            ? new[] { "storage_data1", "data2", "key" }
            : new[] { "storage_data1", "Data2", "Key" };
...
#pragma warning restore Moq1002 // Moq: No matching constructor
        ...
        this._searchClientMock.Verify(
            x => x.UploadDocumentsAsync(
                It.Is<IEnumerable<MultiPropsModel>>(x => x.Count() == 1 && x.First().
                    Key == TestRecordKey1),
...
#pragma warning restore Moq1002 // Moq: No matching constructor
        ...
        this._searchClientMock.Verify(
            x => x.UploadDocumentsAsync(
                It.Is<IEnumerable<MultiPropsModel>>(x => x.Count() == 2 && x.First().
                    Key == TestRecordKey1 && x.ElementAt(1).
                    Key == TestRecordKey2),
                It.Is<IndexDocumentsOptions>(x => x.ThrowOnAnyError == true),
                this._testCancellationToken),
            Times.Once);
...
#pragma warning restore Moq1002 // Moq: No matching constructor
        ...
        mapperMock.Setup(x => x.MapFromDataToStorageModel(It.IsAny<MultiPropsModel>())).
    ...
    private AzureAISearchVectorStoreRecordCollection<MultiPropsModel> CreateRecordCollection(bool useDefinition, bool useCustomJsonSerializerOptions = false)
    {
        return new AzureAISearchVectorStoreRecordCollection<MultiPropsModel>(
            ...
            new()
            {
                VectorStoreRecordDefinition = useDefinition
                    ? this._multiPropsDefinition
                    : null,
                JsonSerializerOptions = useCustomJsonSerializerOptions
                    ? this._customJsonSerializerOptions
                    : null
            });
    }
