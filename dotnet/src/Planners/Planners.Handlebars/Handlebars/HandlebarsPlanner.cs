...
public sealed class HandlebarsPlanner
{
    ...
    public Task<HandlebarsPlan> CreatePlanAsync(
        Kernel kernel,
        string goal,
    ...
    private async Task<HandlebarsPlan> CreatePlanCoreAsync(
        Kernel kernel,
        string goal,
        KernelArguments? arguments,
        CancellationToken cancellationToken = default)
    {
        ...
        var functionsMetadata = await kernel.Plugins.GetFunctionsAsync(this._options, null, null, cancellationToken).
        ...
        var createPlanPrompt = await this.GetHandlebarsTemplateAsync(kernel, goal, arguments, availableFunctions,
                complexParameterTypes, complexParameterSchemas, cancellationToken).
            ConfigureAwait(false);
        ...
        var completionResults = await chatCompletionService.GetChatMessageContentAsync(chatMessages, executionSettings: this._options.ExecutionSettings, cancellationToken: cancellationToken).
            ConfigureAwait(false);
        ...
        {
            var functionNames = availableFunctions.ToList().
                Select(func => $"{func.PluginName}{this._templateFactory.NameDelimiter}{func.Name}");
        ...
        var planTemplate = match.Groups[2].
            Value.Trim();
        ...
        CancellationToken cancellationToken)
    {
        ...
        var additionalContext = this._options.GetAdditionalPromptContext is not null
            ? await this._options.GetAdditionalPromptContext.Invoke().
                ConfigureAwait(false)
            : null;
        ...
        return await handlebarsTemplate!.RenderAsync(kernel, arguments, cancellationToken).
            ConfigureAwait(true);
    }
