...
public sealed class FunctionCallingStepwisePlannerOptions : PlannerOptions
{
    ...
    internal int? MaxCompletionTokens => (this.MaxTokens is null)
        ? null
        : (int)(this.MaxTokens * this.MaxTokensRatio);

    internal int? MaxPromptTokens => (this.MaxTokens is null)
        ? null
        : (int)(this.MaxTokens * (1 - this.MaxTokensRatio));
