...
public class ChatCompletionAgentTests
{
    ...
    public async Task ItShouldReturnChatCompletionServiceMessagesAsync()
    {
        ...
        mockChatCompletionService.Setup(ccs => ccs.GetChatMessageContentsAsync(It.IsAny<ChatHistory>(), It.IsAny<PromptExecutionSettings>(), It.IsAny<Kernel>(), It.IsAny<CancellationToken>())).
