...
internal sealed class ChatThread : IAgentThread
{
    ...
    public async Task<IChatMessage> AddUserMessageAsync(string message, IEnumerable<string>? fileIds = null, CancellationToken cancellationToken = default)
    {
        ...
        var messageModel = await this._restContext.CreateUserTextMessageAsync(this.Id, message, fileIds, cancellationToken).
            ConfigureAwait(false);
    ...
    {
        return this.InvokeAsync(agent, string.Empty, arguments, null,
            cancellationToken);
    }


    /// <inheritdoc/>
    public async IAsyncEnumerable<IChatMessage> InvokeAsync(
        IAgent agent,
        string userMessage,
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            yield return await this.AddUserMessageAsync(userMessage, fileIds, cancellationToken).
                ConfigureAwait(false);
        }

        // Finalize prompt / agent instructions using provided parameters.
        var instructions = await agent.AsPromptTemplate().
            RenderAsync(agent.Kernel, arguments, cancellationToken).
            ConfigureAwait(false);

        // Create run using templated prompt
        var runModel = await this._restContext.CreateRunAsync(this.Id, agent.Id, instructions, agent.Tools,
                cancellationToken).
            ConfigureAwait(false);
        ...
        await foreach (var messageId in run.GetResultAsync(cancellationToken).
                           ConfigureAwait(false))
        {
            var message = await this._restContext.GetMessageAsync(this.Id, messageId, cancellationToken).
                ConfigureAwait(false);
    ...
    public async Task DeleteAsync(CancellationToken cancellationToken)
    {
        ...
        await this._restContext.DeleteThreadModelAsync(this.Id, cancellationToken).
            ConfigureAwait(false);
