...
internal sealed class NativeFunction : ISKFunction
{
    ...
    /// <inheritdoc/>
    public FunctionView Describe() => _view ??= new FunctionView(this.Name, this.PluginName, this.Description, this._parameters, this._returnParameter);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Invoke the function.
            var result = await _function(null, requestSettings, context, cancellationToken).ConfigureAwait(false);
    ...
    private FunctionResult CallFunctionInvoked(FunctionResult result, SKContext context)
    {
        ...
        if (eventWrapper?.Handler is EventHandler<FunctionInvokedEventArgs> handler)
        {
            ...
            result = new FunctionResult(Name, PluginName, eventWrapper.EventArgs.SKContext, eventWrapper.EventArgs.SKContext.Result)
            {
