...
internal sealed class KernelFunctionFromMethod : KernelFunction
{
        ...
        ref bool hasCultureParam)
    {
        ...
        if (type == typeof(ILogger) || type == typeof(ILoggerFactory))
        {
            ...
            return type == typeof(ILogger) ? ((Kernel kernel, KernelArguments _, CancellationToken _) => kernel.GetService<ILoggerFactory>().CreateLogger(method?.DeclaringType ?? typeof(KernelFunctionFromPrompt)), null) : ((Kernel kernel, KernelArguments _, CancellationToken _) => kernel.GetService<ILoggerFactory>(), null);
        }
