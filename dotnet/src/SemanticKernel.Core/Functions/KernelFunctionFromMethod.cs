...
internal sealed class KernelFunctionFromMethod : KernelFunction
{
    ...
    private static (Func<Kernel, ContextVariables, CancellationToken, object?>, KernelParameterMetadata?) GetParameterMarshalerDelegate(
        ...
        ref bool hasContextVariablesParam,
        ...
        ref bool hasCultureParam)
    {
        ...
        if (type == typeof(ILogger) || type == typeof(ILoggerFactory))
        {
            ...
            return type == typeof(ILogger) ? ((Kernel kernel, ContextVariables context, CancellationToken _) => kernel.GetService<ILoggerFactory>().CreateLogger(method?.DeclaringType ?? typeof(KernelFunctionFromPrompt)), null) : ((Kernel kernel, ContextVariables context, CancellationToken _) => kernel.GetService<ILoggerFactory>(), null);
        }
