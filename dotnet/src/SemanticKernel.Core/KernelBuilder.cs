...
public sealed class KernelBuilder : IKernelBuilder
{
    ...
    public IKernelBuilderPlugins Plugins => _plugins ??= new KernelBuilderPlugins(Services);
    ...
    public Kernel Build()
    {
        ...
        if (_services is { Count: > 0 } services)
        {
            ...
            foreach (ServiceDescriptor serviceDescriptor in services)
            {
                ...
                {
                    typeToKeyMappings[serviceDescriptor.ServiceType] = keys = new HashSet<object>();
                }
    ...
    {
        public KernelBuilderPlugins(IServiceCollection services) => Services = services;
