...
internal static class PostgresSqlBuilder
{
        ...
        Dictionary<VectorPropertyModel, IReadOnlyList<Embedding>>? generatedEmbeddings)
    {
        ...
        foreach (var record in records)
        {
            ...
            batchCommand.CommandText = autoGeneratedKey
                ? upsertSqlWithAutoGeneratedKey ??= GenerateSingleUpsertSql(true)
                : upsertSql ??= GenerateSingleUpsertSql(false);
            ...
            foreach (var property in model.Properties)
            {
                ...
                batchCommand.Parameters.Add(npgsqlDbType.HasValue
                    ? new NpgsqlParameter { Value = value ?? DBNull.Value, NpgsqlDbType = npgsqlDbType.Value }
        ...
        where TKey : notnull
    {
        ...
        sql.AppendLine()
            ...
            .AppendLine()
            .Append("WHERE ")
            .AppendIdentifier(model.KeyProperty.StorageName)
            .Append(" = $1;");
    ...
    /// </summary>
    internal static StringBuilder AppendIdentifier(
        ...
        double? scoreThreshold = null)
    {
        ...
        var (where, parameters) = GenerateFilterClause(model,
            filter,
            2);
        ...
        sql.Append("SELECT ")
            ...
            .Append(distanceOp)
            .Append(" $1 AS ")
            .AppendIdentifier(PostgresConstants.DistanceColumnName)
            .AppendLine()
            .Append("FROM ")
            .AppendTableName(schema, tableName)
            .Append(' ')
            .AppendLine(where)
            .Append("ORDER BY ")
            .AppendLine(PostgresConstants.DistanceColumnName)
            .Append("LIMIT ")
            .Append(limit);
        ...
        if (scoreThreshold.HasValue)
        {
            ...
            outerSql.Append("SELECT * FROM (")
                .Append(commandText)
                .Append(") AS scored WHERE ")
                .AppendIdentifier(PostgresConstants.DistanceColumnName)
                .Append(' ')
                .Append(comparisonOp)
                .Append(" $")
                .Append(scoreThresholdParamIndex);
            commandText = outerSql.ToString();
        }
        ...
        double? scoreThreshold = null)
    {
        ...
        var (filterCondition, filterParameters) = GenerateFilterCondition(model,
            filter,
            4);
        ...
        StringBuilder sql = new();
        sql.AppendLine()
            .Append("WITH semantic_search AS (")
            .AppendLine()
            .Append("    SELECT ")
            .AppendIdentifier(model.KeyProperty.StorageName)
        ...
        sql.AppendLine()
            ...
            .Append("    SELECT ")
            .AppendIdentifier(model.KeyProperty.StorageName)
            .Append(", RANK () OVER (ORDER BY ts_rank_cd(to_tsvector(")
            .AppendLiteral(language)
            .Append(", ")
            .AppendIdentifier(textProperty.StorageName)
            .Append("), query) DESC) AS rank")
            .AppendLine()
            .Append("    FROM ")
            .Append(fullTableName)
        ...
        sql.AppendLine()
            .Append("    ORDER BY ts_rank_cd(to_tsvector(")
            .AppendLiteral(language)
            .Append(", ")
            .AppendIdentifier(textProperty.StorageName)
            .Append("), query) DESC")
            ...
            .Append(',')
            .AppendLine()
            .Append("    COALESCE(1.0 / ($3 + semantic_search.rank), 0.0) +")
            .AppendLine()
            .Append("    COALESCE(1.0 / ($3 + keyword_search.rank), 0.0) AS ")
            .AppendIdentifier(PostgresConstants.DistanceColumnName)
            .AppendLine()
            .Append("FROM semantic_search")
            .AppendLine()
            .Append("FULL OUTER JOIN keyword_search ON semantic_search.")
            .AppendIdentifier(model.KeyProperty.StorageName)
            ...
            .Append(" t ON t.")
            .AppendIdentifier(model.KeyProperty.StorageName)
            .Append(" = COALESCE(semantic_search.")
            .AppendIdentifier(model.KeyProperty.StorageName)
            .Append(", keyword_search.")
            .AppendIdentifier(model.KeyProperty.StorageName)
            .Append(')')
