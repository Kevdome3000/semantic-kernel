...
#pragma warning restore CA1711 // Identifiers should not have incorrect suffix
{
    ...
    public CosmosNoSqlCollection(Database database, string name, CosmosNoSqlCollectionOptions? options = default)
        : this(new(database.Client, ownsClient: false),
            _ => database,
            name,
            options)
    {
    ...
    public CosmosNoSqlCollection(
        ...
        : this(
            new ClientWrapper(new CosmosClient(connectionString, clientOptions), true),
        ...
        CosmosNoSqlCollectionOptions? options)
    {
        ...
        try
        {
            ...
            _indexingMode = options.IndexingMode;
            _automatic = options.Automatic;
            ...
            _mapper = typeof(TRecord) == typeof(Dictionary<string, object?>)
                ? (new CosmosNoSqlDynamicMapper(_model, jsonSerializerOptions) as ICosmosNoSqlMapper<TRecord>)!
                : new CosmosNoSqlMapper<TRecord>(_model, options.JsonSerializerOptions);
            ...
            if (options.PartitionKeyProperties is null)
            {
                ...
                _partitionKeyProperties = [_model.KeyProperty];
            }
            else
            {
                ...
                _partitionKeyProperties = new List<PropertyModel>(options.PartitionKeyProperties.Count);

                foreach (var propertyName in options.PartitionKeyProperties)
                {
                    if (!_model.PropertyMap.TryGetValue(propertyName, out var property))
                    {
                    ...
                    _partitionKeyProperties.Add(property);
                }
            }
            ...
            if ((typeof(TKey) == typeof(string) || typeof(TKey) == typeof(Guid))
                && (_partitionKeyProperties is not [var singlePkProperty] || singlePkProperty != _model.KeyProperty))
            {
                throw new ArgumentException(
                    $"When using {typeof(TKey).Name} as the key type, the partition key must be the key property " + $"('{_model.KeyProperty.ModelName}'). To use a different partition key configuration, use CosmosNoSqlKey as the key type.");
            }
    ...
    public override async Task<bool> CollectionExistsAsync(CancellationToken cancellationToken = default)
    {
        ...
        var queryDefinition = new QueryDefinition(Query).WithParameter("@collectionName", Name);

        using var feedIterator = VectorStoreErrorHandler.RunOperation<FeedIterator<string>, CosmosException>(
            _collectionMetadata,
            OperationName,
            () => _database.GetContainerQueryIterator<string>(queryDefinition));

        using var errorHandlingFeedIterator = new ErrorHandlingFeedIterator<string>(feedIterator, _collectionMetadata, OperationName);
    ...
    public override async Task EnsureCollectionExistsAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await _database.CreateContainerAsync(GetContainerProperties(), cancellationToken: cancellationToken).ConfigureAwait(false);
        }
    ...
    public override async Task<TRecord?> GetAsync(TKey key, RecordRetrievalOptions? options = null, CancellationToken cancellationToken = default)
    {
        ...
        return jsonObject is null
            ? default
            : _mapper.MapFromStorageToDataModel(jsonObject, includeVectors);
    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var items = keys.Select(GetDocumentIdAndPartitionKey).ToList();
        ...
        {
            var record = _mapper.MapFromStorageToDataModel(jsonObject, includeVectors);
    ...
    private async Task UpsertCoreAsync(
        TRecord record,
        int recordIndex,
        IReadOnlyList<MEAI.Embedding>?[]? generatedEmbeddings,
        CancellationToken cancellationToken = default)
    {
        ...
        var partitionKey = BuildPartitionKey(record);

        var jsonObject = _mapper.MapFromDataToStorageModel(record, recordIndex, generatedEmbeddings);

        var keyValue = jsonObject.TryGetPropertyValue(_model.KeyProperty.StorageName!, out var jsonKey)
            ? jsonKey?.ToString()
            : null;
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var record in MapSearchResultsAsync(searchResults,
                ScorePropertyName,
                OperationName,
                options.IncludeVectors,
                cancellationToken)
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var (whereClause, filterParameters) = new CosmosNoSqlFilterTranslator().Translate(filter, _model);
        ...
        {
            var record = _mapper.MapFromStorageToDataModel(jsonObject, options.IncludeVectors);
    ...
    {
        return VectorStoreErrorHandler.RunOperationAsync<T, CosmosException>(
            _collectionMetadata,
            operationName,
            operation);
    }
    ...
    private ContainerProperties GetContainerProperties()
    {
        ...
        var containerProperties = new ContainerProperties(
            Name,
            partitionKeyPaths: _partitionKeyProperties.Count > 0
                ? _partitionKeyProperties.Select(p => $"/{p.StorageName}").ToList()
                : ["/id"])
        {
            IndexingPolicy = indexingPolicy
        };

        if (_indexingMode == IndexingMode.None)
        {
    ...
    {
        return (Nullable.GetUnderlyingType(vectorDataType) ?? vectorDataType) switch
        {
    ...
    private async IAsyncEnumerable<T> GetItemsAsync<T>(QueryDefinition queryDefinition, string operationName, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        using var errorHandlingFeedIterator = new ErrorHandlingFeedIterator<T>(feedIterator, _collectionMetadata, operationName);
    ...
    {
        switch (_partitionKeyProperties)
        {
            case []:
            ...
            default:
                ...
                foreach (var property in _partitionKeyProperties)
                {
