...
#pragma warning restore CA1711 // Identifiers should not have incorrect suffix
{
    ...
    internal CosmosMongoCollection(
        IMongoDatabase mongoDatabase,
        string name,
        Func<CosmosMongoCollectionOptions, CollectionModel> modelFactory,
        CosmosMongoCollectionOptions? options)
    {
        // Verify.
...
#pragma warning disable CS0618 // VectorSearchFilter is obsolete
        var filter = options switch
        {
            ...
            { OldFilter: VectorSearchFilter legacyFilter } => CosmosMongoCollectionSearchMapping.BuildFilter(legacyFilter, _model),
            { Filter: Expression<Func<TRecord, bool>> newFilter } => new CosmosMongoFilterTranslator().Translate(newFilter, _model),
            _ => null
        };
#pragma warning restore CS0618
        ...
        var searchQuery = vectorPropertyIndexKind switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"Index kind '{vectorProperty.IndexKind}' on {nameof(VectorStoreVectorProperty)} '{vectorProperty.StorageName}' is not supported by the Azure CosmosDB for MongoDB VectorStore. " + $"Supported index kinds are: {string.Join(", ", [IndexKind.Hnsw, IndexKind.IvfFlat])}")
        };
        ...
        while (await cursor.MoveNextAsync(cancellationToken).ConfigureAwait(false))
        {
            foreach (var response in cursor.Current)
            {
                if (skipCounter >= searchOptions.Skip)
                {
                    ...
                    var record = _mapper.MapFromStorageToDataModel(response[DocumentPropertyName].AsBsonDocument, searchOptions.IncludeVectors);
