...
internal static class SqliteCommandBuilder
{
    ...
    public static DbCommand BuildCreateTableCommand(
        ...
        bool ifNotExists)
    {
        ...
        foreach (var column in columns)
        {
            if (column.HasIndex)
            {
                ...
                builder.AppendIdentifier($"{tableName}_{column.Name}_index")
                    .Append(" ON ")
        ...
        bool replaceIfExists = false)
    {
        ...
        var properties = model.KeyProperties.Concat(data
                ? model.DataProperties
                : (IEnumerable<PropertyModel>)model.VectorProperties)
            .ToList();
        ...
        int skip = 0)
    {
        ...
        var (command, whereClause) = GetCommandWithWhereClause(connection,
            conditions,
            extraWhereFilter,
            extraParameters);
        ...
        double? scoreThreshold = null)
    {
        ...
        var queryExtraFilter = new StringBuilder()
            .AppendIdentifier(vectorTableName)
            .Append('.')
            .AppendIdentifier(keyColumnName)
            .Append(" IN (SELECT ")
            .AppendIdentifier(keyColumnName)
            .Append(" FROM ")
            .Append(SubqueryName)
        ...
        builder.Append("SELECT ");
        builder.AppendColumnNames(includeVectors: true,
            model.Properties,
            vectorTableName,
            dataTableName);
        ...
        builder.Append("INNER JOIN ")
            .AppendIdentifier(dataTableName)
            .Append(" ON ")
            .AppendIdentifier(vectorTableName)
            .Append('.')
            .AppendIdentifier(keyColumnName)
        ...
        if (scoreThreshold.HasValue)
        {
            var scoreThresholdClause = new StringBuilder()
                .AppendIdentifier(vectorTableName)
                .Append('.')
                .AppendIdentifier(DistancePropertyName)
    ...
    private static StringBuilder AppendOrderBy<TRecord>(
        this StringBuilder builder,
        CollectionModel model,
        ...
        string? tableName = null)
    {
        ...
        if (orderBy is { Count: > 0 })
        {
            ...
            foreach (var sortInfo in orderBy)
            {
                ...
                builder.AppendIdentifier(storageName)
    ...
    private static string GetColumnDefinition(SqliteColumn column, bool quote, bool includeNullability)
    {
        ...
        List<string> columnDefinitionParts =
        [
            quote
