...
public abstract class AgentFactory
{
    ...
    {
        return Types.Any(s => string.Equals(s, agentDefinition.Type, StringComparison.OrdinalIgnoreCase));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var kernelAgent = await TryCreateAsync(kernel,
                agentDefinition,
    ...
    /// <param name="cancellationToken">Optional cancellation token.</param>
    /// <return>The created <see cref="Agent"/>, if null the agent type is not supported.</return>
    public abstract Task<Agent?> TryCreateAsync(
        Kernel kernel,
        AgentDefinition agentDefinition,
        AgentCreationOptions? agentCreationOptions = null,
        CancellationToken cancellationToken = default);
