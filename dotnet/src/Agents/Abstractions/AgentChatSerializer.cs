...
public sealed class AgentChatSerializer
{
    ...
    public static async Task<AgentChatSerializer> DeserializeAsync(Stream stream, JsonSerializerOptions? serializerOptions = null)
    {
        AgentChatState state =
            await JsonSerializer.DeserializeAsync<AgentChatState>(stream, serializerOptions ?? s_defaultOptions).ConfigureAwait(false) ?? throw new KernelException("Unable to restore chat: invalid format.");
    ...
    /// </summary>
    public IEnumerable<AgentParticipant> Participants => _state.Participants;


    /// <summary>
    /// Restores the <see cref="AgentChat"/> to the previously captured state.
    /// </summary>
    public Task DeserializeAsync<TChat>(TChat chat) where TChat : AgentChat
    {
        return chat.DeserializeAsync(_state);
    }
