...
public sealed class OpenAIResponseAgent : Agent
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        AgentThread agentThread = await EnsureThreadExistsWithMessagesAsync(messages, thread, cancellationToken).ConfigureAwait(false);

        // Get the context contributions from the AIContextProviders.
        OpenAIResponseAgentInvokeOptions extensionsContextOptions = await FinalizeInvokeOptionsAsync(messages,
                options,
                agentThread,
                cancellationToken)
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        async Task NotifyMessagesAsync()
        {
            for (; messageIndex < chatHistory.Count; messageIndex++)
            {
                ...
                await NotifyThreadOfNewMessage(agentThread, newMessage, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<AgentThread> EnsureThreadExistsWithMessagesAsync(ICollection<ChatMessageContent> messages, AgentThread? thread, CancellationToken cancellationToken)
    {
        if (StoreEnabled)
        {
            return await EnsureThreadExistsWithMessagesAsync(messages,
                    thread,
                    () => new OpenAIResponseAgentThread(Client),
                    cancellationToken)
                .ConfigureAwait(false);
        }
    ...
    private async Task<OpenAIResponseAgentInvokeOptions> FinalizeInvokeOptionsAsync(
        ICollection<ChatMessageContent> messages,
...
#pragma warning restore SKEXP0130 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
        ...
        OpenAIResponseAgentInvokeOptions extensionsContextOptions =
            options is null
                ...
                : new OpenAIResponseAgentInvokeOptions(options)
                {
                    AdditionalInstructions = mergedAdditionalInstructions,
                    Kernel = kernel
                };
