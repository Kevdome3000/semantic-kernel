...
{
    private readonly bool _useThreadConstructorExtension;
        ...
        IReadOnlyDictionary<string, string>? metadata = null)
    {
        ...
        _useThreadConstructorExtension = true;

        _client = client;
        _messages = messages;
        _codeInterpreterFileIds = codeInterpreterFileIds;
    ...
    protected override async Task<string?> CreateInternalAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (_useThreadConstructorExtension)
            {
                return await _client.CreateThreadAsync(_messages,
                        _codeInterpreterFileIds,
                        _vectorStoreId,
                        _metadata,
                        cancellationToken)
            ...
            var assistantThreadResponse = await _client.CreateThreadAsync(_options, cancellationToken).ConfigureAwait(false);
    ...
    protected override async Task DeleteInternalAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            await _client.DeleteThreadAsync(Id, cancellationToken).ConfigureAwait(false);
        }
    ...
    protected override async Task OnNewMessageInternalAsync(ChatMessageContent newMessage, CancellationToken cancellationToken = default)
    {
        ...
        if (newMessage.Metadata == null || !newMessage.Metadata.TryGetValue("ThreadId", out var messageThreadId) || !Equals(messageThreadId, Id))
        {
            try
            {
    ...
    public async IAsyncEnumerable<ChatMessageContent> GetMessagesAsync(MessageCollectionOrder? sortOrder = default, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var message in AssistantThreadActions.GetMessagesAsync(_client,
                Id!,
                sortOrder,
                cancellationToken)
            .ConfigureAwait(false))
