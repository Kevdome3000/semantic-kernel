...
internal static class AssistantThreadActions
{
    ...
    public static async IAsyncEnumerable<ChatMessageContent> GetMessagesAsync(AssistantsClient client, string threadId, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        do
        {
            messages = await client.GetMessagesAsync(threadId, limit: 100, ListSortOrder.Descending, after: lastId,
            ...
            foreach (ThreadMessage message in messages)
            {
                ...
                {
                    Assistant assistant = await client.GetAssistantAsync(message.AssistantId, cancellationToken).
                        ConfigureAwait(false);
        ...
        do
        {
            ...
            if (run.Status == RunStatus.RequiresAction)
            {
                ...
                if (activeFunctionSteps.Length > 0)
                {
                    ...
                    await client.SubmitToolOutputsToRunAsync(run, toolOutputs, cancellationToken).
                        ConfigureAwait(false);
                }
            ...
            foreach (RunStep completedStep in completedStepsToProcess)
            {
                ...
                else if (completedStep.Type.Equals(RunStepType.MessageCreation))
                {
                    ...
                    ThreadMessage? message = await RetrieveMessageAsync(messageCreationDetails, cancellationToken).
                        ConfigureAwait(false);
...
#pragma warning disable CA1031 // Do not catch general exception types
                try
                {
                    run = await client.GetRunAsync(threadId, run.Id, cancellationToken).
                        ConfigureAwait(false);
                }
                catch
                {
...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        async Task<ThreadMessage?> RetrieveMessageAsync(RunStepMessageCreationDetails detail, CancellationToken cancellationToken)
        {
            ...
            do
            {
                try
                {
                    message = await client.GetMessageAsync(threadId, detail.MessageCreation.MessageId, cancellationToken).
                        ConfigureAwait(false);
                }
