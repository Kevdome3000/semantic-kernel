...
internal static class AgentToolDefinitionExtensions
{
    ...
    internal static BinaryData GetParameters(this AgentToolDefinition agentToolDefinition)
    {
        ...
        return parameters is not null
            ? CreateParameterSpec(parameters)
            : s_noParams;
    }
    ...
    internal static BinaryData CreateParameterSpec(List<object> parameters)
    {
        ...
        foreach (var parameter in parameters)
        {
            ...
            if (parameterProps is not null)
            {
                ...
                string? name = parameterProps.TryGetValue("name", out var nameValue) && nameValue is string nameString
                    ? nameString
                    : null;
                string? type = parameterProps.TryGetValue("type", out var typeValue) && typeValue is string typeString
                    ? typeString
                    : null;
                string? description = parameterProps.TryGetValue("description", out var descriptionValue) && descriptionValue is string descriptionString
                    ? descriptionString
                    : string.Empty;
    ...
    internal static List<VectorStoreDataSource> CreateDataSources(List<object> values)
    {
        ...
        foreach (var value in values)
        {
            ...
            {
                string? assetIdentifier = dataSourceDict.TryGetValue("asset_identifier", out var identifierValue) && identifierValue is string identifierString
                    ? identifierString
                    : null;
                string? assetType = dataSourceDict.TryGetValue("asset_type", out var typeValue) && typeValue is string typeString
                    ? typeString
                    : null;
    ...
    internal static IList<VectorStoreConfigurations>? GetVectorStoreConfigurations(this AgentToolDefinition agentToolDefinition)
    {
        ...
        return dataSources is not null
            ? CreateVectorStoreConfigurations(dataSources)
            : null;
    }
    ...
    internal static List<VectorStoreConfigurations> CreateVectorStoreConfigurations(List<object> values)
    {
        ...
        foreach (var value in values)
        {
            ...
            {
                var storeName = configurationDict.TryGetValue("store_name", out var storeNameValue) && storeNameValue is string storeNameString
                    ? storeNameString
                    : null;
                var dataSources = configurationDict.TryGetValue("data_sources", out var dataSourceValue) && dataSourceValue is List<object> dataSourceList
                    ? CreateDataSources(dataSourceList)
                    : null;
