...
internal sealed class ChatHistoryChannel : AgentChannel
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (ChatMessageContent responseMessage in historyAgent.InvokeAsync(_history,
                null,
                null,
                cancellationToken)
        ...
        {
            return !message.Items.Any(i => i is FunctionCallContent or FunctionResultContent) || messageQueue.Count == 0;
        }
    }
    ...
    protected override async IAsyncEnumerable<StreamingChatMessageContent> InvokeStreamingAsync(Agent agent, IList<ChatMessageContent> messages, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (StreamingChatMessageContent streamingMessage in historyAgent.InvokeStreamingAsync(_history,
                null,
                null,
                cancellationToken)
            .ConfigureAwait(false))
    ...
    protected override Task ReceiveAsync(IEnumerable<ChatMessageContent> history, CancellationToken cancellationToken)
    {
        ...
        _history.AddRange(
            history.Where(m => !string.IsNullOrEmpty(m.Content) || m.Items.Where(i => s_contentMap.Contains(i.GetType())).Any()));
