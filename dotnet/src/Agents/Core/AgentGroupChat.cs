...
public sealed class AgentGroupChat : AgentChat
{
    ...
    /// </summary>
    public AgentGroupChatSettings ExecutionSettings { get; set; } = new();

    /// <summary>
    /// Gets the agents participating in the chat.
    /// </summary>
    public override IReadOnlyList<Agent> Agents => _agents.AsReadOnly();
    ...
    public override async IAsyncEnumerable<ChatMessageContent> InvokeAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        for (int index = 0; index < ExecutionSettings.TerminationStrategy.MaximumIterations; index++)
        {
            ...
            Agent agent = await SelectAgentAsync(cancellationToken).ConfigureAwait(false);
    ...
    public override async IAsyncEnumerable<StreamingChatMessageContent> InvokeStreamingAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        for (int index = 0; index < ExecutionSettings.TerminationStrategy.MaximumIterations; index++)
        {
            ...
            // Invoke agent and process messages along with termination
            await foreach (var message in InvokeStreamingAsync(agent, cancellationToken).ConfigureAwait(false))
            {
                yield return message;
            }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        Logger.LogAgentGroupChatInvokingAgent(nameof(InvokeAsync),
            agent.GetType(),
            agent.Id,
    ...
    {
        _agents = new List<Agent>(agents);
        _agentIds = new HashSet<string>(_agents.Select(a => a.Id));
    }
    ...
    {
        Logger.LogAgentGroupChatSelectingAgent(nameof(InvokeAsync), ExecutionSettings.SelectionStrategy.GetType());

        Agent agent;

        try
        {
            agent = await ExecutionSettings.SelectionStrategy.NextAsync(Agents, History, cancellationToken).ConfigureAwait(false);
        }
        catch (Exception exception)
        {
            Logger.LogAgentGroupChatNoAgentSelected(nameof(InvokeAsync), exception);
            throw;
        }

        Logger.LogAgentGroupChatSelectedAgent(nameof(InvokeAsync),
            agent.GetType(),
            agent.Id,
            agent.GetDisplayName(),
            ExecutionSettings.SelectionStrategy.GetType());
