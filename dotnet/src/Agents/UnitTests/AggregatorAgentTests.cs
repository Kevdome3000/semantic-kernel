...
public class AggregatorAgentTests
{
    ...
    public async Task VerifyAggregatorAgentUsageAsync(AggregatorMode mode, int modeOffset)
    {
        ...
        var messages = await uberChat.GetChatMessagesAsync().
            ToArrayAsync();

        Assert.Single(messages);

        messages = await uberChat.GetChatMessagesAsync(uberAgent).
            ToArrayAsync();

        Assert.Empty(messages); // Agent hasn't joined chat, no broadcast

        messages = await groupChat.GetChatMessagesAsync().
            ToArrayAsync();
        ...
        messages = await uberChat.GetChatMessagesAsync().
            ToArrayAsync();
        ...
        messages = await uberChat.InvokeAsync(uberAgent).
            ToArrayAsync();
