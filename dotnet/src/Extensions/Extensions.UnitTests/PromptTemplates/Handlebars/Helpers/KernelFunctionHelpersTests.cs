...
public sealed class KernelFunctionHelpersTests
{
    ...
    public async Task ItRendersFunctionHelpersWitHashArgumentsAndInputVariableAsync()
    {
        ...
        var result = await RenderPromptTemplateAsync(template, inputVariables, arguments);
    ...
    public async Task ShouldThrowExceptionWhenArgumentsAreNotProvidedAsync()
    {
        ...
        // Act and Assert
        var exception = await Assert.ThrowsAsync<ArgumentException>(() => RenderPromptTemplateAsync(template));
    ...
    public async Task ShouldThrowExceptionWhenFunctionHasNullPositionalParameterAsync()
    {
        ...
        // Assert
        var exception = await Assert.ThrowsAsync<KernelException>(() => RenderPromptTemplateAsync(template, inputVariables, arguments));
    ...
    private async Task<string> RenderPromptTemplateAsync(string template, List<InputVariable>? inputVariables = null, KernelArguments? arguments = null)
    {
        ...
        // Act
        var result = await target.RenderAsync(_kernel, arguments ?? _arguments);
    ...
    private sealed class Foo
    {
        ...
        public string Combine([Description("First word")] string x, [Description("Second word")] string y)
        {
            return y + x;
        }
        ...
        public string StringifyInt([Description("Number to stringify")] int x)
        {
        ...
        public ChatMessageContent ChatMessageContent(string role, string content)
        {
            return new ChatMessageContent(new AuthorRole(role), content);
        }


        [KernelFunction] [Description("Return RestApiOperationResponse")]
        public RestApiOperationResponse RestApiOperationResponse(string content, string contentType)
        {
            return new RestApiOperationResponse(content, contentType);
        }


        [KernelFunction] [Description("Return CustomReturnType")]
        public CustomReturnType CustomReturnType(string textProperty)
        {
            return new CustomReturnType(textProperty);
        }
    }
