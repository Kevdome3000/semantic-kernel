...
public static class CopilotAgentPluginKernelExtensions
{
    ...
    {
        KernelPlugin plugin = await kernel.CreatePluginFromCopilotAgentPluginAsync(pluginName,
                filePath,
                pluginParameters,
                cancellationToken)
...
#pragma warning restore CA2000
        ...
        foreach (var runtime in openAPIRuntimes)
        {
            ...
            var predicate = OpenApiFilterService.CreatePredicate(string.Join(",", manifestFunctions.Select(static f => f.Name)),
                null,
                null,
                openApiDocument);
