...
public sealed class BingTextSearchTests : IDisposable
{
    ...
    public void AddBingTextSearchSucceeds()
    {
        ...
        // Act
        builder.AddBingTextSearch("ApiKey");
    ...
    {
        // Arrange
        _messageHandlerStub.AddJsonResponse(File.ReadAllText(WhatIsTheSKResponseJson));

        // Create an ITextSearch instance using Bing search
        var textSearch = new BingTextSearch("ApiKey", new BingTextSearchOptions { HttpClient = _httpClient });

        // Act
        KernelSearchResults<string> result = await textSearch.SearchAsync("What is the Semantic Kernel?", new TextSearchOptions { Top = 10, Skip = 0 });
    ...
    public async Task GetSearchResultsReturnsSuccessfullyAsync()
    {
        ...
        // Act
        KernelSearchResults<object> result = await textSearch.GetSearchResultsAsync("What is the Semantic Kernel?", new TextSearchOptions { Top = 10, Skip = 0 });
    ...
    public async Task SearchWithCustomStringMapperReturnsSuccessfullyAsync()
    {
        ...
        // Create an ITextSearch instance using Bing search
        var textSearch = new BingTextSearch("ApiKey", new BingTextSearchOptions { HttpClient = _httpClient, StringMapper = new TestTextSearchStringMapper() });
    ...
    public async Task GetTextSearchResultsWithCustomResultMapperReturnsSuccessfullyAsync()
    {
        ...
        // Create an ITextSearch instance using Bing search
        var textSearch = new BingTextSearch("ApiKey", new BingTextSearchOptions { HttpClient = _httpClient, ResultMapper = new TestTextSearchResultMapper() });
    ...
    {
        // Arrange
        _messageHandlerStub.AddJsonResponse(File.ReadAllText(SiteFilterDevBlogsResponseJson));
        ...
        // Assert
        var requestUris = _messageHandlerStub.RequestUris;
    ...
    public async Task GenericSearchAsyncWithLanguageEqualityFilterProducesCorrectBingQueryAsync()
    {
        // Arrange
        _messageHandlerStub.AddJsonResponse(File.ReadAllText(WhatIsTheSKResponseJson));
        ITextSearch<BingWebPage> textSearch = new BingTextSearch("ApiKey", new BingTextSearchOptions { HttpClient = _httpClient });
    ...
    public async Task GenericSearchAsyncWithMixedEqualityAndContainsProducesCorrectBingQueryAsync()
    {
        ...
        var searchOptions = new TextSearchOptions<BingWebPage>
        {
            Top = 4,
            Skip = 0,
            Filter = page => page.Language == "en" && page.IsFamilyFriendly == true && page.Name!.Contains("Azure") && page.DisplayUrl!.Contains("microsoft.com")
        };
    ...
    public async Task GenericSearchAsyncWithComplexFourConditionFilterProducesCorrectBingQueryAsync()
    {
        ...
        var searchOptions = new TextSearchOptions<BingWebPage>
        {
            ...
            Filter = page => page.Language == "en"
                && page.Language != "fr"
                && // This should be ignored (contradiction)
    ...
    public async Task GenericSearchAsyncWithIsFamilyFriendlyInequalityThrowsExceptionAsync()
    {
        ...
        var exception = await Assert.ThrowsAsync<ArgumentException>(async () => await textSearch.SearchAsync("content", searchOptions));
    ...
    public async Task GenericSearchAsyncWithContainsOnNameAndUrlProducesDistinctOperatorsAsync()
    {
        ...
        Assert.Equal(2, Regex.Matches(uri, "docs").Count);
    }
    ...
    public async Task DefaultMarketIsAppliedToSearchRequestAsync()
    {
        // Arrange
        _messageHandlerStub.AddJsonResponse(File.ReadAllText(WhatIsTheSKResponseJson));
        var textSearch = new BingTextSearch("ApiKey", new BingTextSearchOptions { HttpClient = _httpClient, Market = "en-US" });
    ...
    public async Task DefaultFreshnessIsAppliedToSearchRequestAsync()
    {
        // Arrange
        _messageHandlerStub.AddJsonResponse(File.ReadAllText(WhatIsTheSKResponseJson));
        var textSearch = new BingTextSearch("ApiKey", new BingTextSearchOptions { HttpClient = _httpClient, Freshness = "Month" });
    ...
    public async Task DefaultSafeSearchIsAppliedToSearchRequestAsync()
    {
        // Arrange
        _messageHandlerStub.AddJsonResponse(File.ReadAllText(WhatIsTheSKResponseJson));
        var textSearch = new BingTextSearch("ApiKey", new BingTextSearchOptions { HttpClient = _httpClient, SafeSearch = "Strict" });
    ...
    public async Task DefaultParametersWorkWithGenericInterfaceAsync()
    {
        // Arrange
        _messageHandlerStub.AddJsonResponse(File.ReadAllText(WhatIsTheSKResponseJson));
        ITextSearch<BingWebPage> textSearch = new BingTextSearch("ApiKey", new BingTextSearchOptions { HttpClient = _httpClient, Market = "de-DE" });
    ...
    public async Task DefaultParametersCoexistWithLinqFiltersAsync()
    {
        // Arrange
        _messageHandlerStub.AddJsonResponse(File.ReadAllText(WhatIsTheSKResponseJson));
        ITextSearch<BingWebPage> textSearch = new BingTextSearch("ApiKey",
            new BingTextSearchOptions
            {
                HttpClient = _httpClient,
                Market = "en-US",
    ...
    public async Task DefaultAnswerCountIsAppliedCorrectlyAsync()
    {
        // Arrange
        _messageHandlerStub.AddJsonResponse(File.ReadAllText(WhatIsTheSKResponseJson));
        var textSearch = new BingTextSearch("ApiKey", new BingTextSearchOptions { HttpClient = _httpClient, AnswerCount = 3 });
