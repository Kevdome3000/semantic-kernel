...
public sealed class TaskListPlugin
{
    ...
    public TaskListPlugin(ITaskManagementConnector connector, ILoggerFactory? loggerFactory = null, JsonSerializerOptions? jsonSerializerOptions = null)
    {
        ...
        _logger = loggerFactory?.CreateLogger(typeof(TaskListPlugin)) ?? NullLogger.Instance;
    }
    ...
    {
        TaskManagementTaskList defaultTaskList = await _connector.GetDefaultTaskListAsync(cancellationToken).ConfigureAwait(false) ?? throw new InvalidOperationException("No default task list found.");
