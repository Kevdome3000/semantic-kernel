...
public class OrganizationHierarchyConnector : IOrganizationHierarchyConnector
{
    ...
    public async Task<string> GetManagerEmailAsync(CancellationToken cancellationToken = default) =>
        ((User)await this._graphServiceClient.Me.Manager.Request().
    ...
    {
        IUserDirectReportsCollectionWithReferencesPage directsPage = await this._graphServiceClient.Me.DirectReports.Request().
            GetAsync(cancellationToken).
            ConfigureAwait(false);

        List<User> directs = directsPage.Cast<User>().
            ToList();
