...
#pragma warning disable CS0618 // Obsolete ITextSearch, TextSearchOptions, TextSearchFilter, FilterClause
    ...
    public async Task<KernelSearchResults<string>> SearchAsync(string query, TextSearchOptions? searchOptions = null, CancellationToken cancellationToken = default)
    {
        ...
        TavilySearchResponse? searchResponse = await ExecuteSearchAsync(query,
                searchOptions.Top,
                searchOptions.Skip,
                filters,
                cancellationToken)
...
#pragma warning restore CS0618
    ...
    async Task<KernelSearchResults<string>> ITextSearch<TavilyWebPage>.SearchAsync(string query, TextSearchOptions<TavilyWebPage>? searchOptions, CancellationToken cancellationToken)
    {
        ...
        TavilySearchResponse? searchResponse = await ExecuteSearchAsync(modifiedQuery,
                top,
                skip,
                filters,
                cancellationToken)
    ...
    async Task<KernelSearchResults<TavilyWebPage>> ITextSearch<TavilyWebPage>.GetSearchResultsAsync(string query, TextSearchOptions<TavilyWebPage>? searchOptions, CancellationToken cancellationToken)
    {
        ...
        return new KernelSearchResults<TavilyWebPage>(GetResultsAsWebPageAsync(searchResponse, cancellationToken), totalCount, GetResultsMetadata(searchResponse));
    }
        ...
        if (propertyName != null && value != null)
        {
            ...
            else
            {
                throw new NotSupportedException(
                    $"Property '{propertyName}' cannot be mapped to Tavily API filters. " + $"Supported properties: {string.Join(", ", s_validFieldNames)}. " + "Example: page => page.Topic == \"general\" && page.TimeRange == \"week\"");
            }
        }
        else
        {
    ...
    private static void ProcessMethodCallClause(MethodCallExpression methodCall, List<(string FieldName, object Value)> filters, List<string> queryTerms)
    {
        if (methodCall.Method.Name == "Contains")
        {
            ...
            {
                throw new NotSupportedException(
                    "Collection Contains filters (e.g., array.Contains(page.Property)) are not supported by Tavily Search API. " + "Consider either: (1) performing multiple separate searches for each value, or " + "(2) retrieving broader results and filtering on the client side.");
            }
        }
        else
        {
    ...
    private async Task<TavilySearchResponse?> ExecuteSearchAsync(
        string query,
        int top,
        int skip,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var requestContent = BuildRequestContent(query,
            top,
            skip,
            filters);
    ...
    private async IAsyncEnumerable<TavilyWebPage> GetResultsAsWebPageAsync(TavilySearchResponse? searchResponse, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        if (_searchOptions?.IncludeImages ?? false && searchResponse.Images is not null)
        {
            foreach (var image in searchResponse.Images!)
            {
                ...
                Uri? imageUri = string.IsNullOrWhiteSpace(image.Url)
                    ? null
                    : new Uri(image.Url);
    ...
    private sealed class DefaultTextSearchStringMapper : ITextSearchStringMapper
    {
        ...
        public string MapFromResultToString(object result)
        {
            ...
            if (result is TavilyImageResult imageResult)
            {
...
#pragma warning restore CS0618
    ...
    /// <param name="skip">Number of results to skip.</param>
    /// <param name="filters">Pre-extracted filter key-value pairs.</param>
    private TavilySearchRequest BuildRequestContent(
        string query,
        int top,
        int skip,
        List<(string FieldName, object Value)> filters)
